...
from .lex import parse, format_ast, serialize_ast, deserialize_ast, serialize_cst, deserialize_cst, SyntaxTree
...
class TransformationPipeline:
  ...
  def transform(self, request: TransformRequest) -> TransformResult:
    ...
    output_format = self._determine_format(to_type, request.format_override, request.source_path)
  ...
  def _load_input(self, content: str, input_type: RepType) -> Any:
    ...
    raise TransformationError(f"Unknown input type: {input_type}", input_type, "unknown")
  ...
  def _transform(self, data: Any, from_type: RepType, to_type: RepType) -> Any:
    ...
    raise TransformationError(
      f"No transformation path from {from_type} to {to_type}", from_type, to_type, reason="Unsupported transformation"
    )
  ...
  def _format_output(self, data: Any, output_type: RepType, format: Format, style: Optional[FormatStyle]) -> str:
    ...
    raise TransformationError(f"Cannot format {output_type} as {format}", output_type, format)
