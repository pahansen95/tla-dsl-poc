...
from observability import ObservabilityConfig, JsonHandler, TimeDeltaHandler, EventHandler
from observability.types import EventDict

from lexical.observe import LEX_TOKEN_EMIT, PARSE_RULE_ENTER, PARSE_RULE_EXIT, PARSE_BACKTRACK, AST_NODE_CREATE
...
def create_trace_handler(detailed: bool = False) -> EventHandler:
  ...
  def format_and_print(event: EventDict) -> None:
    ...
    if formatted:  # Skip None results
      ...
      print(f"{timestamp_ms:8.1f}ms (+{delta_us:4.0f}μs) {formatted}", file=sys.stderr)
...
def create_metrics_handler() -> EventHandler:
  ...
  metrics: Dict[str, Any] = {"token_count": 0, "rule_durations": {}, "backtrack_count": 0, "ast_nodes": 0}
...
def create_error_handler() -> EventHandler:
  ...
  def handle_error(event: EventDict) -> None:
    if event["type"] == "error" or event["type"].endswith(".error"):
      ...
      if position:
        print(f"ERROR at line {position.line}, column {position.column}: {error_msg}", file=sys.stderr)
      else:
