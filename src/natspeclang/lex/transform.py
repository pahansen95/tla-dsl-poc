...
Extracts semantic information from concrete syntax trees to build
...
class ASTBuilder:
  ...
  def _extract_states(self, spec_node: NodeView) -> list[StateDeclaration]:
    ...
    for state_block in self._find_all_children(state_list, "state_block"):
      ...
      states.append(StateDeclaration(name=name, properties=tuple(properties), initial_condition=initial_condition))
  ...
  def _extract_operations(self, spec_node: NodeView) -> list[Operation]:
    ...
    for op_node in self._find_all_children(spec_node, "operation"):
      ...
      operations.append(
        Operation(
          trigger=trigger_text, preconditions=tuple(preconditions), effects=tuple(effects), unchanged=tuple(unchanged)
        )
      )
  ...
  def _extract_property(self, prop_node: NodeView, prop_type: str) -> Optional[Property]:
    ...
    return Property(name=name, content=content, property_type=prop_type)
